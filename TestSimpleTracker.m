...
n_points_per_frame = 2022; 

%% Create the random points
points = cell(n_frames, 1);
frame_point= zeros(2225,2);
for current_frame=1:n_frames;

str=num2str(current_frame);
xPosition = csvread(strcat('AF52M8DL_F00000084',num2str(current_frame,'%04d'),'_channel=0001_frame=0001_statistics.csv'),1,5);
%yPosition = csvread(strcat('AF52M8DL_F00000084',num2str(current_frame,'%04d'),'_channel=0001_frame=0001_statistics.csv'),1,6);
frame_point(1:size(xPosition),1)=xPosition(:,1);
frame_point(1:size(xPosition),2)=xPosition(:,2);
points{current_frame} = frame_point;
end
